# Teoría de la Computación

Repositorio personal de la materia **Teoría de la Computación** (Computabilidad
y Complejidad), cursada en la **Universidad Nacional de Hurlingham (UNaHur)**.

| | |
|---|---|
| **Materia** | Teoría de la Computación — Computabilidad y Complejidad |
| **Carrera** | Licenciatura en Informática |
| **Año de cursada** | 2026 — 2° año |
| **Alumno** | Luciano Robles |
| **Docente** | Mg. Ing. Pablo Pandolfo |
| **Universidad** | Universidad Nacional de Hurlingham (UNaHur) |

> **Objetivo de la materia:** entender qué problemas se pueden resolver con una
> computadora, cuántos recursos se necesitan para ello y cuáles son los límites
> reales del cálculo.

Material de la cátedra: [github.com/ppandomail/tc](https://github.com/ppandomail/tc)

---

## Sobre este repositorio

La cursada se hace resolviendo los trabajos prácticos de la cátedra. Acá subo
mis resoluciones, apuntes y cualquier implementación que acompañe la teoría,
para tener registro ordenado del recorrido y material de repaso para los
parciales.

## Programa

| Unidad | Tema | Contenido de la cátedra |
|---|---|---|
| 01 | Introducción | [Intro](https://github.com/ppandomail/tc/blob/master/doc/intro.ipynb) |
| 02 | Máquina de Turing | [MTr](https://github.com/ppandomail/lf/blob/main/doc/mt.ipynb) |
| 02 | Máquina de Turing de cinta múltiple | [MTc](https://github.com/ppandomail/tc/blob/master/doc/mtc.ipynb) |
| 02 | Máquina de Turing Universal | [MTU](https://github.com/ppandomail/tc/blob/master/doc/mtu.ipynb) |
| 02 | Problemas y lenguajes | [Prob](https://github.com/ppandomail/tc/blob/master/doc/prob.ipynb) |
| 02 | Máquina secuencial | [Mseq](https://github.com/ppandomail/tc/blob/master/doc/mseq.ipynb) |
| 02 | Redes de Petri | [PN](https://github.com/ppandomail/tc/blob/master/doc/pn.ipynb) |
| 02 | Autómatas de Células de McCulloch-Pitts | [MP](https://github.com/ppandomail/tc/blob/master/doc/mp.ipynb) |
| 02 | Autómatas Finitos Probabilísticos | [AFP](https://github.com/ppandomail/tc/blob/master/doc/afp.ipynb) |
| 03 | Complejidad computacional | [Complejidad](https://github.com/ppandomail/tc/blob/master/doc/comple.ipynb) |
| 04 | Computación cuántica | [Cuántica](https://github.com/ppandomail/tc/blob/master/doc/cuanti.ipynb) |

## Ejercitación

Cada entrega lleva su número. Si es un archivo suelto (una planilla, un link),
va el link directo; si es un conjunto de archivos, se vera el link a la carpeta de este repo.

1. [Cientificos](https://docs.google.com/spreadsheets/d/19Bz7HpqyY465uTSSMZl_xUvJH9eviDKqdAx2mwPVTQA/edit?gid=0#gid=0)
   : La primera entrega de la materia. Se resuelve directamente sobre la planill.

2. **Máquina de Turing** — [`Ejercitacion 2/`](Ejercitacion%202/)
   Máquina de Turing calculadora que duplica la cantidad de unos.

3. **Máquina de Turing** — [`Ejercitacion 3/`](Ejercitacion%203/)
   Máquinas de Turing para un lenguaje regular y uno independiente del contexto,
   más el comparativo entre MTaccept y MTcalc.

## Bibliografía

- *Teoría de autómatas, lenguajes y computación* — Hopcroft, Motwani y Ullman
- *Computabilidad, complejidad computacional y verificación de programas* — Rosenfeld
- *Maquinaria computacional*

## Herramientas

- [JFLAP](https://www.jflap.org/) — autómatas y máquinas de Turing
- [Turing Machine Simulator](https://turingmachinesimulator.com)

---

_Repositorio de uso académico. Las resoluciones son propias y pueden contener errores._
